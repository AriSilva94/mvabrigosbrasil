...
export async function GET(
  ...
  if (cursor) {
    // Cursor composto "created_at|id" para paginação estável
    // (evita pular mensagens com mesmo timestamp)
    ...
    if (separatorIdx !== -1) {
      ...
      // Fallback para formato antigo (só created_at)
  ...
  return NextResponse.json({
    ...
    next_cursor:
      hasMore && result[0]
        ? `${result[0].created_at}|${result[0].id}`
        : null,
  });
}
...
export async function POST(
  ...
  // 6. Sanitizar (remover HTML — múltiplas passadas para prevenir bypass)
