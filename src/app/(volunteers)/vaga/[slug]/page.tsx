...
async function loadVacancy(slug: string): Promise<{
  ...
  if (postType === REGISTER_TYPES.shelter && shelterId) {
    ...
    if (ownVacancy) {
      ...
      return { vacancy: mapped, canEdit: true, hasApplied: false, isAuthenticated, threadId: null, shelterProfileId: null };
    }
  }

  const publicVacancy = await getPublicVacancyBySlug(slug);
  if (!publicVacancy) {
    return { vacancy: null, canEdit: false, hasApplied: false, isAuthenticated, threadId: null, shelterProfileId: null };
  }
  ...
  if (isAuthenticated && publicVacancy.id) {
    ...
    if (volunteer) {
      ...
      // Buscar thread do chat para esta vaga + voluntário (inclui shelter_profile_id para online indicator)
      if (hasApplied) {
        const { data: thread } = await supabaseAdmin
          .from("chat_threads")
          .select("id, shelter_profile_id")
          .eq("vacancy_id", publicVacancy.id)
          .eq("volunteer_profile_id", userId)
          .maybeSingle();

        threadId = thread?.id ?? null;
        shelterProfileId = thread?.shelter_profile_id ?? null;
      }
    }
  }

  return { vacancy: publicVacancy, canEdit: false, hasApplied, isAuthenticated, threadId, shelterProfileId };
}
...
export default async function Page({
  ...
  const { slug } = await params;
  const { vacancy, canEdit, hasApplied, isAuthenticated, threadId, shelterProfileId } = await loadVacancy(slug);
