...
export async function loadDatabaseDatasetNew(): Promise<DatabaseDataset> {
  const { shelters: shelterRows, dynamics: dynamicsRows } = await fetchSupabaseData();

  // Processar abrigos
  const shelters: ShelterRecord[] = shelterRows.map((shelter) => {
    ...
    const dateToUse = shelter.created_at;

    return {
      id: shelter.wp_post_id ?? 0,
      title: shelter.name ?? "",
      postDate: dateToUse ?? "",
