...
export default function ChatThread({
  ...
  const { user } = useAuth();
  const { messages, isLoading, hasMore, error, fetchMessages, addMessage, replaceOptimistic, updateMessage, sendMessage, editMessage } = useChatMessages();
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const initialLoadRef = useRef(false);
  // Rastrear IDs de mensagens enviadas por este usuário para ignorar no Realtime
  const sentMessageIdsRef = useRef<Set<string>>(new Set());
  // Rastrear IDs de mensagens editadas por este usuário para ignorar no Realtime
  const editedMessageIdsRef = useRef<Set<string>>(new Set());
  ...
  const handleNewMessage = useCallback(
    (message: Parameters<typeof addMessage>[0]) => {
      // Ignorar TODAS as mensagens do próprio usuário via Realtime.
      // O remetente já tem o optimistic update e recebe a versão real
      // via resposta da API (replaceOptimistic). Isso elimina a race condition
      // onde o Realtime chegava antes da API retornar o ID real.
    ...
    [addMessage, threadId, onUnreadCountUpdate, user?.id]
  );

  // Realtime: mensagem editada ou status atualizado (read)
  const handleMessageUpdated = useCallback(
    (message: ChatMessage) => {
      // Se é update de status (read) nas PRÓPRIAS mensagens, sempre aplicar
      if (message.sender_id === user?.id && message.status) {
        updateMessage(message.id, { status: message.status });
        return;
      }
      // Ignorar edições feitas por este usuário (já temos via optimistic update)
      if (editedMessageIdsRef.current.has(message.id)) {
        editedMessageIdsRef.current.delete(message.id);
        return;
      }
  ...
  useChatRealtime(threadId, handleNewMessage, handleMessageUpdated);
  ...
  const handleSaveEdit = useCallback((messageId: string, content: string) => {
    // Optimistic update
    updateMessage(messageId, { content, edited_at: new Date().toISOString() });
    editedMessageIdsRef.current.add(messageId);
    setEditingMessage(null);

    // Envio real em background
    editMessage(threadId, messageId, content).then((result) => {
      if (result) {
        updateMessage(messageId, result);
      }
    });
  }, [threadId, updateMessage, editMessage]);
