...
export default function CookieBanner() {
  ...
  const [currentConsent, setCurrentConsent] = useState<ConsentCategories>(
    () => {
      ...
      return (
        parseConsent(getCookieValue(COOKIE_CONSENT_NAME)) ?? DEFAULT_CONSENT
      );
    },
  );
        ...
        >
          <div className="mx-auto flex max-w-5xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm leading-relaxed text-text-default">
              Utilizamos cookies para melhorar sua experiência. Apenas os
              cookies necessários são carregados por padrão. Você pode
              personalizar suas preferências ou consultar nossa{" "}
              <a
