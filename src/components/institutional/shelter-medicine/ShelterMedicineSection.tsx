...
import { SHELTER_MEDICINE_CTA_LINKS } from "@/constants/medicinaDeAbrigos";
    ...
    <section aria-labelledby="shelter-medicine-title" className="bg-white">
      <div className="container px-6 py-12">
        <article className="mx-auto max-w-4xl space-y-6 text-base leading-relaxed text-slate-600">
          ...
          <p>
            As recomendações em medicina de abrigos podem diferir da prática
            privada devido às considerações para toda a população e não
            individualmente. A MA é uma área da medicina veterinária do coletivo
            (MVC) – Especialidade da Medicina Veterinária reconhecida pelo
            Conselho Federal de Medicina Veterinária em 2021 – que está em
            ascensão no Brasil, que tem como objetivo promover o manejo
            populacional de cães e gatos em áreas urbanas (MPCG), sustentável e
            ético, e promover a reabilitação e ressocialização dos animais
            abandonados para que sejam reintroduzidos na sociedade por meio da
            adoção responsável.
          </p>

          <figure className="overflow-hidden rounded-3xl">
            <Image
              src="https://mvabrigosbrasil.com.br/wp-content/uploads/2023/08/medicina-de-abrigos.jpg"
          ...
          <nav aria-label="Links relacionados">
            <ul className="flex flex-wrap gap-4">
              {SHELTER_MEDICINE_CTA_LINKS.map(({ href, label, icon }) => (
                <li key={href}>
                  <Link
                    href={href}
                    className="inline-flex items-center gap-2 rounded-full border border-brand-primary px-4 py-2 font-semibold text-brand-primary transition hover:bg-brand-primary hover:text-white"
                  >
