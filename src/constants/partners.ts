export const PARTNERS_INTRO =
  "A iniciativa Medicina de Abrigos Brasil – Infodados de Abrigos de Animais surgiu como uma forma de suprir a necessidade de promover a ciência da Medicina de abrigos no país, e obter dados representativos com base em estatísticas nacionais para o desenvolvimento de políticas públicas que reduzam o abandono de animais de estimação e promovam a adoção.";

export const PARTNERS_FOR_SHELTERS =
  "Caso você seja um abrigo (seja ele público, privado ou misto) ou um lar temporário/protetor independente que resgata e abriga cães e gatos, e quer nos ajudar registrando seus dados da dinâmica populacional mensal, cadastre-se. Ao participar como um contribuidor de dados, você terá acesso a um banco de dados objetivo e imparcial de suas próprias estatísticas do abrigo e também de outras instituições, tanto a nível local quanto nacional. Você estará ajudando a melhorar a compreensão sobre a realidade dos animais abandonados e em situação de risco, na construção de políticas públicas mais eficazes e a alocar melhor os recursos em vários abrigos ou provedores de serviços para animais.";

export const PARTNERS_FOR_ORGANIZATIONS =
  "Caso você seja uma organização/empresa privada ou pública e se interessou pelo projeto, compactua com a nossa missão e objetivos, e queira nos ajudar com contribuições e doações, entre em contato conosco via e-mail.";

export const PARTNERS_CONTACT_EMAIL = "contato@mvabrigosbrasil.com.br";

export type PartnerLogo = {
  name: string;
  image: string;
  alt?: string;
};

export const PARTNER_REALIZERS: PartnerLogo[] = [
  {
    name: "Universidade Federal do Paraná",
    image: "/assets/img/parceiros/logo-ufpr.png",
  },
  {
    name: "Programa de Pós-graduação em Ciências Veterinárias",
    image: "/assets/img/parceiros/logo-ppgcv.png",
  },
  {
    name: "Medicina Veterinária do Coletivo",
    image: "/assets/img/parceiros/logo-mvc.png",
  },
];

export const PARTNER_FINANCIERS: PartnerLogo[] = [
  {
    name: "Fundação Araucária",
    image: "/assets/img/parceiros/fundacao_araucaria.png",
  },
  {
    name: "SETI",
    image: "/assets/img/parceiros/SETI.png",
  },
  {
    name: "SEDEST",
    image: "/assets/img/parceiros/SEDEST.png",
  },
];

export const PARTNER_SUPPORTERS: PartnerLogo[] = [
  {
    name: "IMVC",
    image: "/assets/img/parceiros/IMVC2.jpg",
  },
  {
    name: "Fórum Animal",
    image: "/assets/img/parceiros/FORUM.png",
  },
  {
    name: "Projeto Medicina Veterinária de Abrigos do Instituto PremieRpet",
    image: "/assets/img/parceiros/MEDVET.png",
  },
  {
    name: "Peritus Vet",
    image: "/assets/img/parceiros/logo_peritus_vet.png",
  },
  {
    name: "Grupo de Pesquisa em Ética Animal",
    image: "/assets/img/parceiros/logo(1).png",
  },
  {
    name: "Grupo de Pesquisa em Ética Animal",
    image: "/assets/img/parceiros/logo(2).png",
  },
  {
    name: "Grupo de Pesquisa em Ética Animal",
    image: "/assets/img/parceiros/logo(3).png",
  },
  {
    name: "Grupo de Estudos em Ética e Direito dos Animais",
    image: "/assets/img/parceiros/logo(4).png",
  },
  {
    name: "Organização de Proteção Animal",
    image: "/assets/img/parceiros/logo(5).png",
  },
  {
    name: "Instituto Caramelo",
    image: "/assets/img/parceiros/caramelo.jpg",
  },
  {
    name: "Febraca",
    image: "/assets/img/parceiros/febraca2.jpg",
  },
  {
    name: "AAC&T",
    image: "/assets/img/parceiros/aacet22.jpg",
  },
  {
    name: "Shelter Dogs MECA",
    image: "/assets/img/parceiros/MECA2.jpg",
  },
  {
    name: "ONG Animal",
    image: "/assets/img/parceiros/IMG_5524-scaled-e1768678371453.png",
  },
];

export const PARTNER_SPONSORS: PartnerLogo[] = [
  {
    name: "PremieRpet",
    image: "/assets/img/parceiros/PREMIERVET2.jpg",
  },
  {
    name: "Cobasi Essencial para a vida",
    image: "/assets/img/parceiros/cobasi.jpg",
  },
  {
    name: "Associação Brasileira de Reciclagem Animal",
    image: "/assets/img/parceiros/ABRA.jpg",
  },
];
